...
- CRITICAL — mechanical observed vs specified: Some certs print "Min" or "Max" in the column HEADER (e.g. "Yield Strength (PSI) Min"), then show two data rows: the first row is the REQUIRED MINIMUM (specification), the second row is the ACTUAL TEST RESULT (observed). Always extract the ACTUAL TEST RESULT row as "observed" and the REQUIRED MINIMUM row as "specified". Never use the specification/minimum row as the observed value. The actual test result will typically be higher than the minimum for tensile/yield and within range for elongation/hardness.
- yield_strength (observed) is the labeled yield/proof value from the ACTUAL TEST row, not from the minimum-requirements row. tensile_strength (observed) is the labeled tensile/ultimate tensile value from the ACTUAL TEST row. If a table has two unlabeled strength values in the observed row, yield_strength is usually the lower value and tensile_strength is usually the higher value.
